$(function () {
    ...
    $("#tab").on("click", "a", function (e) {
        var id = $(this).attr("id");
        $.post("../user/getGVirtualLibrary", {id: id}, function (data) {
            $('#submit').hide();
            if (data.sfhg == "不合格") $(".new .bhgyy").show();
            else $(".new .bhgyy").hide();
            $('#update,#delete').show();
            $('.modal-title').text("查看股库");
            $("#delete").attr("name", id);
            var m = data.mul;
            var h = "";
            bzzIndex = 0;
            ...
            buttons: [
                ...
                },
                {
                    ...
                    callback: function () {
                        ...
                        $.ajax(
                            {
                                url: "../user/deleteGVirtualLibrary",
                                type: "DELETE",
                                data: {id: id}
                            }).success(function () {
    ...
    $('form').formValidation({
        ...
        if (handle == "create") {
            $.ajax({
                url: "../user/createGVirtualLibrary",
                type: 'POST',
            ...
            $.ajax({
                url: "../user/updateGVirtualLibrary",
                type: 'POST',
    ...
    $(".new input[name='djmc'],.new input[name='zj']").on('input propertychange', function () {
        ...
        $(".new input[name='zl']").val(accMul(zj, zj, djmc, 0.617) / 100);
    });
